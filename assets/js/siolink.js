(function (window) {
  ...
  function attachLinkLayer() {
    ...
    if (!g || !g.sio) return;
    g.sio.linkLayer = {
      startNormal32Transfer(txData) { sendSio32(txData); },
      setBaud() {}  // unused mais requis par l'interface
    };
    log("linkLayer attaché au SIO");
  }

  function detachLinkLayer() {
    const g = gba();
    if (g && g.sio && g.sio.linkLayer && g.sio.linkLayer._isSioLink) {
      g.sio.linkLayer = null;
    }
  }

  /* ---- Échange SIO32 ----------------------------------------- */
  // Appelé par sio.js (côté maître) quand writeSIOCNT démarre un transfert.
  function sendSio32(txData) {
    if (!channel || channel.readyState !== "open") return;
    channel.send(JSON.stringify({ t: "sio", d: txData >>> 0 }));
    log("SIO TX → " + (txData >>> 0).toString(16));
  }
  ...
  function onChannelMessage(evt) {
    ...
    if (msg.t === "sio") {
      // Données du pair reçues (maître → esclave ou esclave → maître)
      const remoteData = msg.d >>> 0;
      log("SIO RX ← " + remoteData.toString(16));

      if (!isMaster) {
        // Côté esclave : lire nos données TX et les renvoyer
        const ourTx = g.sio.readTxData32();
        channel.send(JSON.stringify({ t: "sio_ack", d: ourTx >>> 0 }));
        // Compléter notre transfert avec les données du maître
        g.sio.completeNormal32Transfer(remoteData);
      } else {
        // Côté maître : recevoir la réponse de l'esclave
        g.sio.completeNormal32Transfer(remoteData);
      }
    }

    if (msg.t === "sio_ack" && isMaster) {
      // Maître reçoit les données de l'esclave
      const remoteData = msg.d >>> 0;
      log("SIO ACK ← " + remoteData.toString(16));
      g.sio.completeNormal32Transfer(remoteData);
    }

    if (msg.t === "ping") {
      channel.send(JSON.stringify({ t: "pong" }));
    }
  }
